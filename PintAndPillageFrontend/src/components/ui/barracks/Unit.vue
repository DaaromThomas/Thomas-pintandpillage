      ...
      created() {
        ...
        computed: {
            ...
            unitMaxProduce: function () {
                ...
                for (let resourceName in this.unit.unit.resourcesRequiredToProduce) {
                    if (this.villageResources[resourceName]){
                        let tempMax = this.villageResources[resourceName] / this.unit.unit.resourcesRequiredToProduce[resourceName];
                        currentMax = Math.min(currentMax, tempMax);
                    }else{
                        return 0;
                    }
                }
                let tempMaxPopulation = this.villagePopulationLeft/this.unit.unit.populationRequiredPerUnit
        ...
        methods:{
            createUnit: function () {
                if (this.sliderValue > 0){
                    let unitToCreate = {"productionBuildingId": this.building.buildingId, "unitType": this.unit.unit.unitName, "amount": this.sliderValue};
                    this.$store.dispatch('createUnit', unitToCreate).then(()=>{
            ...
            unitLevelRequirementMet: function () {
                let unitName = this.unit.unit.unitName;
                for (let i = 0; i < this.unitUnlockList.length; i++){
                    if (this.unitUnlockList[i].unitType === unitName && this.unitUnlockList[i].level > this.building.level){
            ...
            unitResearchRequirementMet: function () {
                let requiredResearch = this.unit.unit.researchRequired;
                let completedResearches = this.village.completedResearches;
            ...
            getUnitAmount: function (unitName) {
                let unitsList = this.village.unitsInVillage;
...
<style lang="scss">
    .disabledOverlay{
        .resourceItemComp, .unitsInStore, .unitInputContainer, p, img, h1, h2, button, .populationFrameComp, .timeComp{
            filter: grayscale(1);
            color: #7f7f7f;
    ...
    .unitContainer{
        ...
        .unitDescription{
            ...
            .unitLevelRequirements, .unitResearchRequirements{
